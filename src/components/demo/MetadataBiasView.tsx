    ...
    <div className="flex items-start gap-3 py-3 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="font-medium text-sm text-slate-900">{indicator.name}</span>
          <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${riskColors[indicator.riskLevel]}`}>
            {indicator.riskLevel}
          </span>
        </div>
        <p className="text-sm text-slate-600">{indicator.explanation}</p>
        {indicator.details && (
          <p className="text-xs text-slate-500 mt-1">{indicator.details}</p>
        )}
      </div>
    </div>
  );
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center py-20">
          <div className="flex flex-col items-center gap-3 text-slate-500">
            ...
            <span className="text-xs text-slate-400">Fetching trial metadata and running bias analysis</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center py-20">
          <div className="max-w-md text-center">
            ...
            <p className="text-sm font-medium text-slate-900 mb-1">Analysis failed</p>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between sticky top-0 bg-white/95 backdrop-blur-sm py-4 -mx-4 px-4 z-10 border-b border-slate-100">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-900 max-w-xl">
            Registry Metadata Analysis
          </h1>
          <p className="text-sm text-slate-500">{trial.conditions.join(", ")}</p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className="px-3 py-1 text-xs font-medium bg-slate-200 text-slate-700 rounded-full">
            Metadata Only
          </span>
          <span className="text-xs text-slate-400">{trial.phase} &bull; {trial.studyType}</span>
        </div>
      </div>
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            ...
            <p className="text-sm font-medium text-slate-900">{trial.allocation}</p>
          </div>
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <p className="text-xs text-slate-500 mb-1">Masking</p>
            <p className="text-sm font-medium text-slate-900">{trial.masking}</p>
          </div>
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <p className="text-xs text-slate-500 mb-1">Results Posted</p>
            <p className="text-sm font-medium text-slate-900">{trial.hasResults ? "Yes" : "No"}</p>
          </div>
        </div>
      </WalkthroughSection>
      ...
      <WalkthroughSection title="Publication Bias Assessment">
        ...
        <p className="text-sm text-slate-600 mt-3">{analysis.publicationBias.summary}</p>
      </WalkthroughSection>
      ...
      <WalkthroughSection title="Selection Bias Assessment">
        ...
        <p className="text-sm text-slate-600 mt-3">{analysis.selectionBias.summary}</p>
      </WalkthroughSection>
      ...
      <WalkthroughSection title="Overall Assessment">
        <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm text-slate-500">Overall Bias Risk Score</p>
              <p className="text-3xl font-bold text-slate-900">{analysis.overallScore.toFixed(1)}</p>
            </div>
