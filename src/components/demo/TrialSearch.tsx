...
const statusColors: Record<string, string> = {
  ...
  "ACTIVE, NOT RECRUITING": "bg-indigo-100 text-indigo-700",
  TERMINATED: "bg-red-100 text-red-700",
  WITHDRAWN: "bg-slate-100 text-slate-600",
  SUSPENDED: "bg-amber-100 text-amber-700",
  "NOT YET RECRUITING": "bg-slate-100 text-slate-500",
};
...
function TrialCard({ trial, onSelect }: { trial: TrialSummary; onSelect: (nctId: string) => void }) {
  ...
  return (
    <button
      ...
      className="w-full text-left p-4 rounded-lg border border-slate-200 bg-white hover:border-slate-400 hover:shadow-md transition-all"
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex-1 min-w-0">
          <span className="text-xs font-mono text-slate-400">{trial.nctId}</span>
          <h4 className="text-sm font-medium text-slate-900 mt-0.5 line-clamp-2">
            {trial.title}
          </h4>
        </div>
        {trial.status && (
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
        ...
        {trial.phase && trial.enrollment && <span className="text-slate-300">|</span>}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <button
          onClick={onBack}
          className="text-sm text-slate-500 hover:text-primary mb-2 flex items-center gap-1"
        >
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
          />
        </div>
        <button
          type="submit"
          disabled={loading || !query.trim()}
          className="px-5 py-2.5 bg-slate-800 text-white text-sm font-medium rounded-lg hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
      ...
      {error && (
        <div className="p-4 rounded-lg bg-red-50 border border-red-200">
          <p className="text-sm text-red-800 font-medium">Search failed</p>
        ...
        <div className="space-y-4">
          ...
          {results.trials.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-slate-500 text-sm">No trials found for &ldquo;{query}&rdquo;</p>
            </div>
          ) : (
      ...
      {!results && !loading && !error && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-slate-500 mb-1">Search any registered clinical trial</p>
          <p className="text-xs text-slate-400">
