...
function TrialCard({ trial, onSelect }: { trial: TrialSummary; onSelect: (nctId: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {trial.leadSponsor && (
        <div className="flex items-center gap-1.5 mb-2 px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded-md w-fit">
          ...
          <span className="text-xs font-medium text-slate-600 dark:text-slate-300 truncate">{trial.leadSponsor}</span>
        </div>
      )}
